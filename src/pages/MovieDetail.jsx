...
const MovieDetail = () => {
  ...
  return (
    <div className={`min-h-screen pt-16 ${
      ...
      {/* AdBlock Detection for non-logged users */}
      {!user && <AdBlockDetector />}
      
      {/* Banner Ad for non-logged users */}
      {!user && <BannerAd />}
          ...
          <div className="lg:col-span-2">
            ...
            <div className="flex flex-wrap gap-4 mb-6">
              <span className={`px-3 py-1 rounded-full text-sm ${
                theme === 'dark' ? 'bg-purple-600' : 'bg-amber-200 text-amber-800'
              }`}>
                {movie.year}
              </span>
              <span className={`px-3 py-1 rounded-full text-sm ${
                theme === 'dark' ? 'bg-blue-600' : 'bg-blue-200 text-blue-800'
              }`}>
                {movie.genre}
              </span>
              <span className={`px-3 py-1 rounded-full text-sm ${
                theme === 'dark' ? 'bg-green-600' : 'bg-green-200 text-green-800'
              }`}>
                {movie.rating}
              </span>
            </div>
      ...
      {/* PopUnder Ad for non-logged users */}
      {!user && <PopUnderAd />}
    </div>
  );
};
